...
problemNames = {'Tension_compression_spring_design', 'Pressure_vessel_design', ...
    'Design_of_gear_train', 'Corrugated_bulkhead_design', 'Car_side_impact_design'};
...
% # 3 - Design of gear train
% # 4 - Corrugated bulkhead design
% # 5 - Car side impact design
if problemChoice == 0
    problemNumbers = 1:numel(problemNames);
else
    if problemChoice < 1 || problemChoice > numel(problemNames)
        error('problemChoice must be 0, 1, 2, 3, 4, or 5.');
    end
    problemNumbers = problemChoice;
end
...
%% Run RDFO experiments
for p = 1:numel(problemNumbers)
    problemNumber = problemNumbers(p);
    selectedProblemName = problemNames{problemNumber};
    ...
    runSeeds = baseSeed + problemNumber * 1000 + (1:runs);
    ...
    % Evaluate run results and save one submission file for the current problem.
